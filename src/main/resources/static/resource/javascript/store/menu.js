...
function menuUpdate() {
	...
	let len = document.getElementsByClassName("groupName");
	let detailName = document.getElementsByName("detailName");
	let detailPrice = document.getElementsByName("detailPrice");
	for (let i = 0; i < len.length; i++) {
		if (len[i].value != "") {
			for (let j = 0; j < detailName.length; j++) {
				if (detailName[j].value == "" || detailPrice[j].value == "") {
					alert("필수 옵션의 이름 혹은 가격이 빠졌습니다.");
					alert(j + "번쨰 " + detailName[j].value);
					alert(j + "번쨰 " + detailPrice[j].value);
					isOk = false;
					break;
				} else {
					detailName[j].setAttribute('value', i + detailName[j].value);
					detailPrice[j].setAttribute('value', i + detailPrice[j].value);
				}
			}
		}
		if (isOk == false) {
